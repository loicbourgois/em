...
def main():
    ...
    write_force(
        f"{path_main}/gold/02_no_group.sacr",
        content,
    )
    ...
    content = read(f"{path_main}/gold/01.md")
    lines = content.split("\n")
    data = []
    group_size = 3*4
    ...
    for i in range(0, len(lines), stride):
        ...
        content = read(f"{HOME}/github.com/loicbourgois/em/template_1.md").format(
            input=json.dumps(group, indent=2, ensure_ascii=False),
        )
        path = f"{path_main}/{model}/03_{group_i}.md"
        data.append({
            "i": group_i,
            "group": group,
            "path": path,
    ...
    for x in data:
        ...
        write_force(
            ...
            json.dumps(yaml_['groups'], indent=2) + "\n",
        )
        input_part2.append(yaml_['groups'])
    write_force(
        f"{path_main}/{model}/06.json",
        json.dumps(input_part2, indent=2) + "\n",
    )
    write_force(
        f"{path_main}/{model}/07.md",
        read(f"{HOME}/github.com/loicbourgois/em/template_2.md").format(
            input=read(f"{path_main}/{model}/06.json")
        )
    )
    
    
    # r = oai.get(read(f"{path_main}/{model}/07.md"), "gpt-5.5-medium")
    # write_force(
    #     f"{path_main}/{model}/08.md",
    #     r['response'],
    # )


    txt = read(f"{path_main}/{model}/08.md",)
    yaml_ = yaml.safe_load(txt.replace("```yaml", "").replace("```json", "").replace("```", ""))
    yaml_['groups_count'] = len(yaml_['groups'])
    write_force(
        f"{path_main}/{model}/09.json",
        json.dumps(yaml_, indent=2)
    )
    txts = []
    for i, x in enumerate(data):
        if i%3 == 0:
            path_in = f"{path_main}/{model}/04_{i}.md.llm.sacr"
            path_out = f"{path_main}/{model}/10_{i}.md.llm.sacr"
            txt = read(path_in)
            for k, v in yaml_['reference to group'].items():
                # v = v.split(".")[-1]
                k = k.split(".")[-1]
    ...
    generate_tokens_and_entities_from_sacr(
        ...
        files_directory=f"{path_main}/{model}",
    )
    silver_df = pandas.read_csv(f"{path_main}/{model}/{silver_name}.entities", sep='\t')
    silver_coreference_matrix = initialize_gold_coreference_matrix_from_entities_df(silver_df)
    coreference_metrics_df = coreference_resolution_metrics(
        gold_coreference_matrix, 
        silver_coreference_matrix,
    )
    coreference_metrics_df.to_csv(f"{path_main}/{model}/00_result.tsv", sep="\t")
